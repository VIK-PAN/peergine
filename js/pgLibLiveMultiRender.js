...
function pgLibLiveMultiRender(oAtx, oUI)
{
	...
	this.VideoStart = function(sCapID, iVideoID, sParam, nodeView) {
		if (!this._bStarted) {
			return pgErrCode.PG_ERR_BadStatus;
		}
	...
	this.AudioSyncDelay = function(sCapID, iAudioID, iVideoID) {
		...
		var sData = "(Item){18}(Value){" + this._oAtx.omlEncode(sObjVideo) + "}";
	...
	this._VideoInit = function(sCapID, iVideoID, sParam, sWndEle) {
		...
		if (!this._oAtx.ObjectAdd(sObjVideo, "PG_CLASS_Live", sObjGroup, iAddFlag)) {
